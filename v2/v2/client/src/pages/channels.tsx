...
export default function Channels() {
  ...
  return (
    <div>
        ...
        <div className="flex items-center space-x-3">
          <div>
            ...
            <p className="text-muted-foreground">
              Подключение и настройка каналов коммуникации
            </p>
          </div>
          <Button
            variant="outline"
            ...
            title="Инструкция по работе с каналами связи"
        ...
        <div className="space-y-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {channels.map((channel) => (
                  <Card key={channel.id} className="bg-green-50">
                    <CardContent className="pt-6 flex flex-col items-center text-center p-6">
                      ...
                      <h3 className="text-lg font-medium text-neutral-900 dark:text-white mb-3 text-center">
                        {channel.name}
                      </h3>
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-amber-700 dark:text-amber-400 mt-1">
                    Вы используете {channelsUsed} из {channelsLimit} доступных
                    каналов в вашем тарифном плане. Для создания дополнительных
                    каналов перейдите на тариф выше.
                  </p>
          ...
          {isBasicPlan && (
            <div className="mt-6 p-4 border border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/30 rounded-lg">
              <div className="flex items-start">
                <span className="material-icons text-blue-600 dark:text-blue-400 mr-3 flex-shrink-0">
                  info
                </span>
                <div>
                  ...
                  <p className="text-sm text-blue-700 dark:text-blue-400 mt-1">
                    На тарифе Базовый доступны только каналы типа Web и
                    Telegram. Для использования других типов каналов перейдите
                    на тариф выше.
                  </p>
                  <Button asChild className="mt-3" variant="outline" size="sm">
                    <Link href="/billing">Изменить тариф</Link>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {CHANNEL_TYPES.filter((channel) => {
                ...
                // Определяем, доступен ли канал для текущего тарифа
                const isChannelAvailable =
                  !isBasicPlan || allowedChannelTypes.includes(channel.id);

                return (
                  <Card
                    key={channel.id}
                    className={!isChannelAvailable ? "opacity-60" : ""}
                  >
                    <CardContent className="pt-6 flex flex-col items-center text-center p-6">
                      <div
                      ...
                      >
                        {!isChannelAvailable
                          ? `Доступно только на тарифе Стандарт и выше`
                          : `Подключите ${channel.name} для общения с вашей аудиторией через этот канал.`}
                      </p>

                      <Button
                        variant={!isChannelAvailable ? "secondary" : "outline"}
                        className="mt-auto w-full"
                        onClick={() => handleConnect(channel)}
                        disabled={!canCreateChannel || !isChannelAvailable}
                      >
                        <span className="material-icons text-[18px] mr-1">
                          {!isChannelAvailable ? "lock" : "add_link"}
                        </span>
                        <span>
                          {!isChannelAvailable ? "Недоступно" : "Подключить"}
                        </span>
                      </Button>
                    </CardContent>
                  </Card>
      ...
      <AvitoChannelDialog
        open={avitoDialogOpen}
        onOpenChange={setAvitoDialogOpen}
      />
